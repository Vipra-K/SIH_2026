...
app=FastAPI(title="BorderSight AI API",version="0.3.0")
app.add_middleware(CORSMiddleware,allow_origins=["*"],allow_credentials=True,allow_methods=["*"],allow_headers=["*"])
app.include_router(stream_router);app.include_router(video_router);app.include_router(camera_source_router);app.include_router(zones_router)
class Severity(str,Enum): low="LOW";medium="MEDIUM";high="HIGH";critical="CRITICAL"
class IncidentStatus(str,Enum): unresolved="UNRESOLVED";acknowledged="ACKNOWLEDGED";resolved="RESOLVED"
class Camera(BaseModel): id:str;name:str;sector:str;source_type:Literal["video","local","rtsp"];status:Literal["ONLINE","OFFLINE"]="ONLINE"
class Incident(BaseModel): id:str;type:str;camera_id:str;sector:str;timestamp:str;object_type:str;track_id:str;confidence:float=Field(ge=0,le=1);risk_score:int=Field(ge=0,le=100);severity:Severity;status:IncidentStatus;reason:list[str]
...
@app.get("/api/health")
def health(): return {"status":"ok","service":"bordersight-api","version":"0.3.0"}
@app.get("/api/dashboard")
def dashboard():
 active=[i for i in incidents if i.status!=IncidentStatus.resolved];return {"cameras":{"total":len(cameras),"online":sum(c.status=="ONLINE" for c in cameras)},"incidents":{"active":len(active),"critical":sum(i.severity==Severity.critical for i in active)},"events_today":147,"sectors":[{"name":"Sector A","status":"CRITICAL","incidents":5},{"name":"Sector B","status":"ELEVATED","incidents":3},{"name":"Sector C","status":"NORMAL","incidents":1}]}
...
def update_incident_status(incident_id:str,body:StatusUpdate):
 ...
 raise HTTPException(404,"Incident not found")
class EventInput(BaseModel): camera_id:str;object_type:str="PERSON";track_id:str;confidence:float=Field(ge=0,le=1);crossed_restricted_boundary:bool=False;loiter_seconds:int=Field(default=0,ge=0);toward_border:bool=False;low_light:bool=False
